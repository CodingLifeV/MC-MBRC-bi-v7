...
import constants
from cross_valuation import DATA_FOLD_PATH, write_data
...
def Pre_Swim(dataset):
    ...
    # 找到 minority 类和 majority 类的标签
    sorted_labels = sorted(label_counts.items(), key=lambda x: x[1])
    minority_label = sorted_labels[0][0]
    majority_label = sorted_labels[-1][0]

    #minority_label, majority_label = [], []
    majority = []
    minority = []
    for x in dataset:
        if x[len(x) - 1] == majority_label:
            majority.append(x)
        elif x[len(x) - 1] == minority_label:
            minority.append(x)
    # print(minority)
    # print(majority)
    #minority, majority
    return minority, majority, minority_label, majority_label
...
class MLOS:
    ...
    def fit_Sample(self, data, labels, numSamples, minority_label, majority_label):

        if self.minClass == None:
            #self.minClass = np.argmin(np.bincount(labels.astype(int)))
            self.minClass = minority_label
       ...
       #bodnum minority class boundary points are 0 and bodnum all point boundaries are shown
        ...
        labels = np.concatenate((
            np.full(len(data_min_orig), minority_label),
            np.full(len(MAJ), majority_label)
        ), axis=0)
...
if __name__ == '__main__':
    ...
    for dataset_name in constants.DATA_NAME:
        for partition in range(1, constants.N_TIMES + 1):
            for fold in range(1, constants.FOLD + 1):
                #path = DATA_FOLD_PATH / 'newthyroid2' / 'newthyroid2.2.1.train.csv'
                path = DATA_FOLD_PATH / dataset_name / ('%s.%d.%d.train.csv' % (dataset_name, partition, fold))

                data = imDataSet(path)
                min, maj, minority_label, majority_label = Pre_Swim(data)
                x_train = data[:, :-1]
                y_train = data[:, -1]
                sw = MLOS(len=1)
                X_res, y_res = sw.fit_Sample(x_train, y_train, len(maj) - len(min), minority_label, majority_label)
                write_data(dataset_name, partition, fold, X_res, y_res, 'TRAIN_RES')
                1
